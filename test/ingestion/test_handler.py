...
def data_retriever():
    ...
    def side_effect():
        result = [
            (index + i, {"question_title": f"Title{index + 1 + i}", "question_body": f"Body{index + 1 + i}", "accepted_answer_body": f"Answer{index + 1 + i}"})
...
def test_ingestion_embedding_failures(embedding_svc, data_retriever, handler):
    ...
    def generate_embedding_intermittent_failures(*args, **kwargs):
        ...
        if call_count % 2 == 0:
            ...
            return [0.1, 0.2, 0.3]
    embedding_svc.generate_embedding.side_effect = generate_embedding_intermittent_failures
