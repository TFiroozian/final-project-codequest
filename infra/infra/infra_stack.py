...
class codeQuestInfraStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        security_group.add_ingress_rule(
            ec2.Peer.ipv4(vpc.vpc_cidr_block), ec2.Port.HTTPS, "Allow inbound HTTPS"
        )
        ...
        ingestion_role = Role.from_role_arn(
            self, "IngestionRole", Fn.import_value("IngestionFunctionIamRole")
        )
        opensearch_cluster.grant_write(ingestion_role)
        self.allow_bedrock_call(ingestion_role, "IngestionRole")

        query_role = Role.from_role_arn(
            self, "QueryRole", Fn.import_value("QueryFunctionIamRole")
        )
        ...
        CfnOutput(
            ...
            value=opensearch_cluster.domain_endpoint,
            export_name="OpensearchDBEndpointURL",
        )
